...
static void show_special(struct audit_context *context, int *call_panic)
{
	...
	case AUDIT_OPENAT2:
		audit_log_format(ab, "oflag=0%llo mode=0%llo resolve=0x%llx",
				 context->openat2.flags, context->openat2.mode,
...
static void audit_log_uring(struct audit_context *ctx)
{
	...
	if (ctx->return_valid != AUDITSC_INVALID)
		audit_log_format(ab, " success=%s exit=%ld",
				 (ctx->return_valid == AUDITSC_SUCCESS ? "yes" :
									 "no"),
				 ctx->return_code);
	audit_log_format(ab,
			 " items=%d"
			 ...
			 ctx->name_count, task_ppid_nr(current),
 ...
 */
static void audit_return_fixup(struct audit_context *ctx, int success,
			       long code)
{
	/*
	...
	if (unlikely(code <= -ERESTARTSYS) &&
	    (code >= -ERESTART_RESTARTBLOCK) && (code != -ENOIOCTLCMD))
...
void __audit_syscall_exit(int success, long return_code)
{
	...
	if (!context || context->dummy || context->context != AUDIT_CTX_SYSCALL)
		goto out;
...
retry:
	...
	if (unlikely(read_seqretry(&rename_lock, seq) ||
		     drop)) { /* in this order */
		rcu_read_unlock();
		if (!drop) {
