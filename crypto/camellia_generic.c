...
static void camellia_setup128(const unsigned char *key, u32 *subkey)
{
	...
	krl = subL[1];
	krr = subR[1];
	CAMELLIA_F(kll, klr, CAMELLIA_SIGMA1L, CAMELLIA_SIGMA1R, w0, w1, il, ir,
		   t0, t1);
	krl ^= w0;
	krr ^= w1;
	CAMELLIA_F(krl, krr, CAMELLIA_SIGMA2L, CAMELLIA_SIGMA2R, kll, klr, il,
		   ir, t0, t1);
	/* current status == (kll, klr, w0, w1) */
	CAMELLIA_F(kll, klr, CAMELLIA_SIGMA3L, CAMELLIA_SIGMA3R, krl, krr, il,
		   ir, t0, t1);
	krl ^= w0;
	krr ^= w1;
	CAMELLIA_F(krl, krr, CAMELLIA_SIGMA4L, CAMELLIA_SIGMA4R, w0, w1, il, ir,
		   t0, t1);
	kll ^= w0;
	klr ^= w1;
...
static void camellia_setup256(const unsigned char *key, u32 *subkey)
{
	...
	krrl ^= krl;
	krrr ^= krr;
	CAMELLIA_F(krll, krlr, CAMELLIA_SIGMA5L, CAMELLIA_SIGMA5R, w0, w1, il,
		   ir, t0, t1);
	krrl ^= w0;
	krrr ^= w1;
	CAMELLIA_F(krrl, krrr, CAMELLIA_SIGMA6L, CAMELLIA_SIGMA6R, w0, w1, il,
		   ir, t0, t1);
	krll ^= w0;
	krlr ^= w1;
...
#define ROUNDS(i)                                                              \
	({                                                                     \
		CAMELLIA_ROUNDSM(io[0], io[1], SUBKEY_L(i + 2),                \
				 SUBKEY_R(i + 2), io[2], io[3], il, ir);       \
		CAMELLIA_ROUNDSM(io[2], io[3], SUBKEY_L(i + 3),                \
				 SUBKEY_R(i + 3), io[0], io[1], il, ir);       \
		CAMELLIA_ROUNDSM(io[0], io[1], SUBKEY_L(i + 4),                \
				 SUBKEY_R(i + 4), io[2], io[3], il, ir);       \
		CAMELLIA_ROUNDSM(io[2], io[3], SUBKEY_L(i + 5),                \
				 SUBKEY_R(i + 5), io[0], io[1], il, ir);       \
		CAMELLIA_ROUNDSM(io[0], io[1], SUBKEY_L(i + 6),                \
				 SUBKEY_R(i + 6), io[2], io[3], il, ir);       \
		CAMELLIA_ROUNDSM(io[2], io[3], SUBKEY_L(i + 7),                \
				 SUBKEY_R(i + 7), io[0], io[1], il, ir);       \
	})
#define FLS(i)                                                                 \
	({                                                                     \
		CAMELLIA_FLS(io[0], io[1], io[2], io[3], SUBKEY_L(i + 0),      \
			     SUBKEY_R(i + 0), SUBKEY_L(i + 1),                 \
			     SUBKEY_R(i + 1), t0, t1, il, ir);                 \
	})
...
#define ROUNDS(i)                                                              \
	({                                                                     \
		CAMELLIA_ROUNDSM(io[0], io[1], SUBKEY_L(i + 7),                \
				 SUBKEY_R(i + 7), io[2], io[3], il, ir);       \
		CAMELLIA_ROUNDSM(io[2], io[3], SUBKEY_L(i + 6),                \
				 SUBKEY_R(i + 6), io[0], io[1], il, ir);       \
		CAMELLIA_ROUNDSM(io[0], io[1], SUBKEY_L(i + 5),                \
				 SUBKEY_R(i + 5), io[2], io[3], il, ir);       \
		CAMELLIA_ROUNDSM(io[2], io[3], SUBKEY_L(i + 4),                \
				 SUBKEY_R(i + 4), io[0], io[1], il, ir);       \
		CAMELLIA_ROUNDSM(io[0], io[1], SUBKEY_L(i + 3),                \
				 SUBKEY_R(i + 3), io[2], io[3], il, ir);       \
		CAMELLIA_ROUNDSM(io[2], io[3], SUBKEY_L(i + 2),                \
				 SUBKEY_R(i + 2), io[0], io[1], il, ir);       \
	})
#define FLS(i)                                                                 \
	({                                                                     \
		CAMELLIA_FLS(io[0], io[1], io[2], io[3], SUBKEY_L(i + 1),      \
			     SUBKEY_R(i + 1), SUBKEY_L(i + 0),                 \
			     SUBKEY_R(i + 0), t0, t1, il, ir);                 \
	})
...
static int camellia_set_key(struct crypto_tfm *tfm, const u8 *in_key,
...
static struct crypto_alg camellia_alg = {
	...
	.cra_u = { .cipher = { .cia_min_keysize = CAMELLIA_MIN_KEY_SIZE,
