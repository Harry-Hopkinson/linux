...
static inline int copy_present_page(struct vm_area_struct *dst_vma,
				    ...
				    pte_t *dst_pte, pte_t *src_pte,
				    unsigned long addr, int *rss,
				    struct page **prealloc, pte_t pte,
				    struct page *page)
{
 ...
 */
static inline bool zap_skip_check_mapping(struct zap_details *details,
					  struct page *page)
{
	if (!details || !page)
		return false;
	...
	while (pages_to_write_in_pmd) {
		...
		for (pte = start_pte; pte_idx < batch_size; ++pte, ++pte_idx) {
			int err = insert_page_in_batch_locked(
				mm, pte, addr, pages[curr_page_idx], prot);
			if (unlikely(err)) {
...
static vm_fault_t wp_page_copy(struct vm_fault *vmf)
{
	...
	if (is_zero_pfn(pte_pfn(vmf->orig_pte))) {
		new_page =
			alloc_zeroed_user_highpage_movable(vma, vmf->address);
		if (!new_page)
			goto oom;
	} else {
		new_page =
	...
	if (!page) {
		if (data_race(si->flags & SWP_SYNCHRONOUS_IO) &&
			...
			if (page) {
				...
				if (mem_cgroup_swapin_charge_page(
					    page, vma->vm_mm, GFP_KERNEL,
					    entry)) {
					ret = VM_FAULT_OOM;
					goto out_page;
				}
...
static vm_fault_t do_fault_around(struct vm_fault *vmf)
{
	...
	end_pgoff = start_pgoff -
		    ...
		    PTRS_PER_PTE - 1;
	end_pgoff =
		min3(end_pgoff, vma_pages(vmf->vma) + vmf->vma->vm_pgoff - 1,
...
static vm_fault_t do_fault(struct vm_fault *vmf)
{
	...
	if (!vma->vm_ops->fault) {
		...
		if (unlikely(!pmd_present(*vmf->pmd)))
			...
			vmf->pte =
				pte_offset_map_lock(vmf->vma->vm_mm, vmf->pmd,
						    vmf->address, &vmf->ptl);
			/*
