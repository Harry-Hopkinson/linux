...
static struct linux_binfmt flat_format = { .module = THIS_MODULE,
...
static int create_flat_tables(struct linux_binprm *bprm,
...
static unsigned long calc_reloc(unsigned long r, struct lib_info *p, int curid,
				int internalp)
{
	unsigned long addr;
	int id;
	...
	if (curid != id) {
		...
		if (p->lib_list[id].build_date &&
		    p->lib_list[curid].build_date &&
		    p->lib_list[curid].build_date <
			    p->lib_list[id].build_date) {
	...
	if (r > start_brk - start_data + text_len) {
		pr_err("reloc outside program 0x%lx (0 - 0x%lx/0x%lx)", r,
		       start_brk - start_data + text_len, text_len);
		goto failed;
	}
...
static int load_flat_file(struct linux_binprm *bprm, struct lib_info *libinfo,
			  int id, unsigned long *extra_stack)
{
	struct flat_hdr *hdr;
	...
	*/
	...
	if (!IS_ENABLED(CONFIG_MMU) &&
		...
		textpos = vm_mmap(bprm->file, 0, text_len,
		...
		datapos = ALIGN(realdatastart + DATA_START_OFFSET_WORDS *
		...
		{
			result =
				read_code(bprm->file, datapos, fpos, full_data);
		}
		...
		reloc = (__be32 __user *)(datapos +
					  (ntohl(hdr->reloc_start) - text_len));
		memp = realdatastart;
		memp_size = len;
	} else {
...
#ifndef CONFIG_MMU
			result = decompress_exec(
				bprm, sizeof(struct flat_hdr),
	...
	if (flags & FLAT_FLAG_KTRACE) {
		pr_info("Mapping is %lx, Entry point is %x, data_start is %x\n",
			textpos, 0x00ffffff & ntohl(hdr->entry),
			ntohl(hdr->data_start));
		pr_info("%s %s: TEXT=%lx-%lx DATA=%lx-%lx BSS=%lx-%lx\n",
			id ? "Lib" : "Load", bprm->filename, start_code,
			end_code, datapos, datapos + data_len,
			datapos + data_len,
	...
	libinfo->lib_list[id].entry =
		(0x00ffffff & ntohl(hdr->entry)) + textpos;
	...
	for (i = 0; i < MAX_SHARED_LIBS; i++) {
		...
		for (j = 0; j < MAX_SHARED_LIBS; j++) {
			...
			unsigned long __user *p =
				(unsigned long __user *)libinfo.lib_list[i]
					.start_data;
			p -= j + 1;
			if (put_user(val, p))
				return -EFAULT;
