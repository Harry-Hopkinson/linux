...
static int create_elf_tables(struct linux_binprm *bprm,
			     const struct elfhdr *exec, unsigned long load_addr,
...
static int load_elf_binary(struct linux_binprm *bprm)
{
	...
	if (interpreter) {
		elf_entry =
			load_elf_interp(interp_elf_ex, interpreter, load_bias,
					interp_elf_phdata, &arch_state);
				 ...
				 long signr, size_t *total)
{
	...
	*total += notesize(&t->notes[0]);
	...
	for (i = 1; i < view->n; ++i) {
		...
		fill_note(&t->notes[i], is_fpreg ? "CORE" : "LINUX", note_type,
			  ret, data);
