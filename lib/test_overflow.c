...
DEFINE_TEST_ARRAY(u8) = {
	...
	{ 1U << 4, 1U << 3, 3 * (1U << 3), 1U << 3, 1U << 7, false, false,
	  false },
...
DEFINE_TEST_ARRAY(u16) = {
	...
	{ 1U << 8, 1U << 7, 3 * (1U << 7), 1U << 7, 1U << 15, false, false,
	  false },
...
DEFINE_TEST_ARRAY(u32) = {
	...
	{ 1U << 16, 1U << 15, 3 * (1U << 15), 1U << 15, 1U << 31, false, false,
	  false },
...
DEFINE_TEST_ARRAY(u64) = {
	...
	{ 1ULL << 31, 1ULL << 31, 1ULL << 32, 0, 1ULL << 62, false, false,
	  false },
	{ 1ULL << 32, 1ULL << 32, 1ULL << 33, 0, 0, false, false, true },
	{ 1ULL << 32, 1ULL << 31, 3 * (1ULL << 31), 1ULL << 31, 1ULL << 63,
	  false, false, false },
	{ 1ULL << 63, 1ULL << 63, 0, 0, 0, true, false, true },
	{ 1000000000ULL /* 10^9 */, 10000000000ULL /* 10^10 */, 11000000000ULL,
	  18446744064709551616ULL, 10000000000000000000ULL, false, true,
	  false },
...
#define check_one_op(t, fmt, op, sym, a, b, r, of)                             \
	do {                                                                   \
		...
		if (_r != r) {                                                 \
			pr_warn("expected " fmt " " sym " " fmt " == " fmt     \
				", got " fmt " (type %s)\n",                   \
				a, b, r, _r, #t);                              \
			err = 1;                                               \
		}                                                              \
...
#define TEST_ONE_SHIFT(a, s, t, expect, of)                                    \
	...
	err |= TEST_ONE_SHIFT(0xFFFFFFFFULL, 32, u64, 0xFFFFFFFFULL << 32,
			      false);
...
#define DEFINE_TEST_ALLOC(func, free_func, want_arg, want_gfp, want_node)      \
	...
	{                                                                      \
		...
		if (!ptr) {                                                    \
			pr_warn(#func                                          \
				" unexpectedly failed bad wrapping?!\n");      \
			return 1;                                              \
		}                                                              \
		free##want_arg(free_func, arg, ptr);                           \
                                                                               \
