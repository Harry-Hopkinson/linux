...
static const u16 token_lookup_tbl[256] = { _W | _A | _C,
					   _W | _A | _C,
					   _W | _A | _C,
					   _W | _A | _C, /*   0-  3 */
					   _W | _A | _C,
...
#define GET_NEXT_BLOCK()                                                       \
	({                                                                     \
		...
		block_idx = 0;                                                 \
		block_len =                                                    \
			conf->get_next_block(consumed, &data, conf, state);    \
	})
...
startover:
	...
	for (tok_idx = 0; tok_idx < fsm->ntokens; tok_idx++) {
		...
		case TS_FSM_PERHAPS:
			if (end_of_data() || !match_token(cur, data[block_idx]))
				continue;
			break;
...
static struct ts_ops fsm_ops = { .name = "fsm",
				 .find = fsm_find,
				 .init = fsm_init,
