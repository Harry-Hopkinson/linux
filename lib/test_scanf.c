...
static void __scanf(4, 6) __init
	_test(check_fn fn, const void *check_data, const char *string,
...
#define _check_numbers_template(arg_fmt, expect, str, fmt, n_args, ap)                   \
	do {                                                                             \
		...
		for (; n_args > 0; n_args--, expect++) {                                 \
			...
			pr_debug("\t" arg_fmt "\n", got);                                \
			if (got != *expect) {                                            \
				pr_warn("vsscanf(\"%s\", \"%s\", ...) expected " arg_fmt \
					" got " arg_fmt "\n",                            \
					str, fmt, *expect, got);                         \
				return 1;                                                \
			}                                                                \
...
#define value_representable_in_type(T, val)                                    \
	(is_signed_type(T) ? ((long long)(val) >= type_min(T)) &&              \
				     ((long long)(val) <= type_max(T)) :       \
...
#define random_for_type(T)                                                     \
	((T)(sizeof(T) <= sizeof(u32) ? next_test_random(BITS_PER_TYPE(T)) :   \
...
#define fill_random_array(arr)                                                 \
	do {                                                                   \
		...
		for (i = 0; i < ARRAY_SIZE(arr); i++, neg_pattern >>= 1) {     \
			...
			if (is_signed_type(typeof((arr)[0])) &&                \
			    (neg_pattern & 1))                                 \
				(arr)[i] = -(arr)[i];                          \
		}                                                              \
	} while (0)
 ...
 */
static void __init append_delim(char *str_buf, int *str_buf_pos,
				int str_buf_len, char *fmt_buf,
				int *fmt_buf_pos, int fmt_buf_len,
...
#define test_array_8(fn, check_data, string, fmt, arr)                         \
	do {                                                                   \
		...
		_test(fn, check_data, string, fmt, 8, &(arr)[0], &(arr)[1],    \
		...
		for (i = 0; i < ARRAY_SIZE(expect); i++) {                     \
			if (i != 0)                                            \
				append_delim(test_buffer, &pos, BUF_SIZE,      \
					     ...
					     field_sep);                       \
                                                                               \
			append_fmt(test_buffer, &pos, BUF_SIZE, gen_fmt,       \
				   expect[i]);                                 \
			append_fmt(fmt_buffer, &fmt_pos, BUF_SIZE, "%%%s",     \
				   scan_fmt);                                  \
		}                                                              \
                                                                               \
		...
		for (i = 0; i < ARRAY_SIZE(expect); i++) {                     \
			if (i != 0)                                            \
				append_delim(test_buffer, &pos, BUF_SIZE,      \
					     ...
					     field_sep);                       \
                                                                               \
			val_len = append_fmt(test_buffer, &pos, BUF_SIZE,      \
					     gen_fmt, expect[i]);              \
			append_fmt(fmt_buffer, &fmt_pos, BUF_SIZE, "%%%u%s",   \
				   val_len, scan_fmt);                         \
		}                                                              \
                                                                               \
		test_array_8(fn, expect, test_buffer, fmt_buffer, result);     \
...
static void __init numbers_list_field_width_ll(const char *delim)
{
	...
	numbers_list_fix_width(unsigned long long, "%llx", delim, 16, "llx",
			       check_ull);
	numbers_list_fix_width(unsigned long long, "0x%llx", delim, 18, "llx",
			       check_ull);
...
static void __init numbers_list_field_width_l(const char *delim)
{
	...
	numbers_list_fix_width(unsigned long, "%lx", delim, 16, "lx",
			       check_ulong);
	numbers_list_fix_width(unsigned long, "0x%lx", delim, 18, "lx",
			       check_ulong);
	numbers_list_fix_width(long, "0x%lx", delim, 18, "li", check_long);
#else
	numbers_list_fix_width(unsigned long, "%lu", delim, 10, "lu",
			       check_ulong);
	numbers_list_fix_width(long, "%ld", delim, 11, "ld", check_long);
	numbers_list_fix_width(long, "%ld", delim, 11, "li", check_long);
	numbers_list_fix_width(unsigned long, "%lx", delim, 8, "lx",
			       check_ulong);
	numbers_list_fix_width(unsigned long, "0x%lx", delim, 10, "lx",
			       check_ulong);
...
static void __init numbers_list_field_width_d(const char *delim)
{
	...
	numbers_list_fix_width(unsigned int, "0x%x", delim, 10, "x",
			       check_uint);
...
static void __init numbers_list_field_width_h(const char *delim)
{
	...
	numbers_list_fix_width(unsigned short, "%hx", delim, 4, "hx",
			       check_ushort);
	numbers_list_fix_width(unsigned short, "0x%hx", delim, 6, "hx",
			       check_ushort);
...
static void __init numbers_list_field_width_hh(const char *delim)
{
	...
	numbers_list_fix_width(signed char, "%hhd", delim, 4, "hhd",
			       check_char);
	numbers_list_fix_width(signed char, "%hhd", delim, 4, "hhi",
			       check_char);
	numbers_list_fix_width(unsigned char, "%hhx", delim, 2, "hhx",
			       check_uchar);
	numbers_list_fix_width(unsigned char, "0x%hhx", delim, 4, "hhx",
			       check_uchar);
	numbers_list_fix_width(signed char, "0x%hhx", delim, 4, "hhi",
			       check_char);
}
...
static void __init numbers_list_field_width_val_ll(const char *delim)
{
	...
	numbers_list_val_width(unsigned long long, "%llx", delim, "llx",
			       check_ull);
	numbers_list_val_width(unsigned long long, "0x%llx", delim, "llx",
			       check_ull);
...
static void __init numbers_list_field_width_val_l(const char *delim)
{
	...
	numbers_list_val_width(unsigned long, "0x%lx", delim, "lx",
			       check_ulong);
...
static void __init numbers_list_field_width_val_hh(const char *delim)
{
	...
	numbers_list_val_width(unsigned char, "%hhx", delim, "hhx",
			       check_uchar);
	numbers_list_val_width(unsigned char, "0x%hhx", delim, "hhx",
			       check_uchar);
...
#define test_number_prefix(T, str, scan_fmt, expect0, expect1, n_args, fn)     \
	do {                                                                   \
		...
		T result[2] = { ~expect[0], ~expect[1] };                      \
                                                                               \
		_test(fn, &expect, str, scan_fmt, n_args, &result[0],          \
		      &result[1]);                                             \
	} while (0)
...
static void __init numbers_prefix_overflow(void)
{
	...
	test_number_prefix(signed char, "-1 1", "%1hhd %hhd", 0, 0, 0,
			   check_char);
	...
	test_number_prefix(signed char, "-1 1", "%1hhi %hhi", 0, 0, 0,
			   check_char);
	 ...
	 */
	test_number_prefix(unsigned long long, "0xA7", "%1llx%llx", 0, 0, 1,
			   check_ull);
	test_number_prefix(unsigned long, "0xA7", "%1lx%lx", 0, 0, 1,
			   check_ulong);
	test_number_prefix(unsigned int, "0xA7", "%1x%x", 0, 0, 1, check_uint);
	test_number_prefix(unsigned short, "0xA7", "%1hx%hx", 0, 0, 1,
			   check_ushort);
	test_number_prefix(unsigned char, "0xA7", "%1hhx%hhx", 0, 0, 1,
			   check_uchar);
	 ...
	 */
	test_number_prefix(unsigned long long, "0xA7", "%2llx%llx", 0, 0xa7, 2,
			   check_ull);
	test_number_prefix(unsigned long, "0xA7", "%2lx%lx", 0, 0xa7, 2,
			   check_ulong);
	test_number_prefix(unsigned int, "0xA7", "%2x%x", 0, 0xa7, 2,
			   check_uint);
	test_number_prefix(unsigned short, "0xA7", "%2hx%hx", 0, 0xa7, 2,
			   check_ushort);
	test_number_prefix(unsigned char, "0xA7", "%2hhx%hhx", 0, 0xa7, 2,
			   check_uchar);
...
#define _test_simple_strtoxx(T, fn, gen_fmt, expect, base)                                   \
	do {                                                                                 \
		...
		if (got != (expect)) {                                                       \
			fail = true;                                                         \
			pr_warn(#fn "(\"%s\", %d): got " gen_fmt                             \
				    " expected " gen_fmt "\n",                               \
				test_buffer, base, got, expect);                             \
		} else if (endp != test_buffer + len) {                                      \
			fail = true;                                                         \
			pr_warn(#fn                                                          \
				"(\"%s\", %d) startp=0x%px got endp=0x%px expected 0x%px\n", \
...
#define test_simple_strtoxx(T, fn, gen_fmt, base)                              \
	do {                                                                   \
		int i;                                                         \
                                                                               \
		for (i = 0; i < ARRAY_SIZE(numbers); i++) {                    \
			_test_simple_strtoxx(T, fn, gen_fmt, (T)numbers[i],    \
					     base);                            \
                                                                               \
			if (is_signed_type(T))                                 \
