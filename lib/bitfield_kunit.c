	...
	do {                                                                   \
		{                                                              \
			...
			KUNIT_ASSERT_FALSE_MSG(context, _res != res,           \
					       ...
					       (u64)_res);                     \
			KUNIT_ASSERT_FALSE(                                    \
				context, u##tp##_get_bits(_res, field) != v);  \
		}                                                              \
	} while (0)
	...
	do {                                                                   \
		{                                                              \
			...
			KUNIT_ASSERT_FALSE_MSG(                                \
				context, _res != cpu_to_le##tp(res),           \
				"le" #tp "_encode_bits(" #v ", " #field        \
				") is 0x%llx != 0x%llx",                       \
				(u64)le##tp##_to_cpu(_res), (u64)(res));       \
			KUNIT_ASSERT_FALSE(                                    \
				context, le##tp##_get_bits(_res, field) != v); \
		}                                                              \
	} while (0)
...
#define CHECK_ENC_GET_BE(tp, v, field, res)                                    \
	do {                                                                   \
		{                                                              \
			...
			KUNIT_ASSERT_FALSE_MSG(                                \
				context, _res != cpu_to_be##tp(res),           \
				"be" #tp "_encode_bits(" #v ", " #field        \
				") is 0x%llx != 0x%llx",                       \
				(u64)be##tp##_to_cpu(_res), (u64)(res));       \
			KUNIT_ASSERT_FALSE(                                    \
				context, be##tp##_get_bits(_res, field) != v); \
		}                                                              \
	} while (0)
