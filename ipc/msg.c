...
static inline struct msg_queue *
msq_obtain_object_check(struct ipc_namespace *ns, int id)
{
...
static void ss_wakeup(struct msg_queue *msq, struct wake_q_head *wake_q,
		      bool kill)
{
...
static void freeque(struct ipc_namespace *ns, struct kern_ipc_perm *ipcp)
	__releases(RCU) __releases(&msq->q_perm)
{
		       ...
		       struct ipc64_perm *perm, int msg_qbytes)
{
	...
	ipcp = ipcctl_obtain_check(ns, &msg_ids(ns), msqid, cmd, perm,
				   msg_qbytes);
	if (IS_ERR(ipcp)) {
		err = PTR_ERR(ipcp);
		goto out_unlock1;
...
static int msgctl_info(struct ipc_namespace *ns, int msqid, int cmd,
...
static int msgctl_stat(struct ipc_namespace *ns, int msqid, int cmd,
		       struct msqid64_ds *p)
{
	struct msg_queue *msq;
	int err;
...
static long do_msgsnd(int msqid, long mtype, void __user *mtext, size_t msgsz,
		      int msgflg)
{
	struct msg_queue *msq;
	struct msg_msg *msg;
...
long compat_ksys_msgsnd(int msqid, compat_uptr_t msgp, compat_ssize_t msgsz,
			int msgflg)
{
...
COMPAT_SYSCALL_DEFINE4(msgsnd, int, msqid, compat_uptr_t, msgp, compat_ssize_t,
		       msgsz, int, msgflg)
{
...
static long
do_msgrcv(int msqid, void __user *buf, size_t bufsz, long msgtyp, int msgflg,
...
COMPAT_SYSCALL_DEFINE5(msgrcv, int, msqid, compat_uptr_t, msgp, compat_ssize_t,
		       msgsz, compat_long_t, msgtyp, int, msgflg)
{
...
static int sysvipc_msg_proc_show(struct seq_file *s, void *it)
{
	...
	seq_printf(
		...
		msq->q_cbytes, msq->q_qnum, pid_nr_ns(msq->q_lspid, pid_ns),
		pid_nr_ns(msq->q_lrpid, pid_ns),
		from_kuid_munged(user_ns, msq->q_perm.uid),
		from_kgid_munged(user_ns, msq->q_perm.gid),
		from_kuid_munged(user_ns, msq->q_perm.cuid),
		from_kgid_munged(user_ns, msq->q_perm.cgid), msq->q_stime,
		msq->q_rtime, msq->q_ctime);
